...
SELECT ?indicator ?topic ?source
WHERE {
    ?i
        skos:inScheme classification:indicator ;
        property:topic [ skos:prefLabel ?topic ] ;
        property:source [ skos:prefLabel ?source ] ;
        skos:prefLabel ?indicator .
}
ORDER BY LCASE(?indicator) ?topic ?source
